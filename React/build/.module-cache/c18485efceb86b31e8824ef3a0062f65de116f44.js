var Number = React.createClass({displayName: "Number",
    ...
    shouldComponentUpdate: function(nextProps, nextState) {
        if(this.props.index - 1 == this.props.current){
    ...
    render: function() {
        return React.createElement("li", {className: this.state.className}, this.index)
    }
});
var Numbers = React.createClass({displayName: "Numbers",
    ...
    render: function() {
        ...
        for (var i = 1; i <= this.props.count; i++) {
            var number = React.createElement("li", {className: this.state.className}, i)
            numbers.push(number);
        }
